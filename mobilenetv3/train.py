"""
OPTIMIZED MobileNetV3 Training for 95%+ Accuracy
==================================================
Combines best features from MobileNetV2 script with MobileNetV3 optimizations:
...
9. ✅ MAP calculation and comprehensive evaluation
10. ✅ Visualization plots for research paper
...
MODEL_SAVE_PATH = "checkpoints/best_mobilenetv3_optimized.keras"
BEST_MODEL_PATH = "checkpoints/best_mobilenetv3_optimized_phase2.keras"
LOG_DIR = "./logs"
RESULTS_DIR = "./results"
LABEL_SMOOTHING = 0.08  # Optimized label smoothing

print("=" * 70)
print("🎯 OPTIMIZED MobileNetV3 Training - i3 6th Gen FAST Mode")
print("=" * 70)
...
def create_optimized_model(num_classes):
    """Create optimized MobileNetV3 model with enhanced head"""
...
# Optimized layer unfreezing for MobileNetV3: freeze first 100 layers, unfreeze rest
# MobileNetV3 has a different structure, so we adjust accordingly
total_layers = len(base_model.layers)
freeze_until = max(100, total_layers - 50)  # Keep at least 50 layers trainable
...
print(f"   (Frozen: {freeze_until} layers, Trainable: {total_layers - freeze_until} layers)")
...
print(f"   • MobileNetV3Large architecture (better than V2)")
...
print(f"   • Better layer unfreezing strategy")
